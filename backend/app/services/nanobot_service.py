...
- 配置加载和重载
- 消息总线管理
- Agent 循环管理（使用 AgentLoopAdapter）
- LLM Provider 管理
- 多模态模型调度
...
# 使用 AgentLoopAdapter 替代原生 AgentLoop
from app.extension import (
    AgentLoopAdapter,
    ExtendedConfig,
    get_extended_config,
)
from app.extension.dashscope_provider import DashScopeVisionProvider
...
class NanobotService:
    ...
    使用 AgentLoopAdapter 支持 LangGraph 多模态处理。

    属性:
        config: Nanobot 配置对象
        extended_config: 扩展配置对象
        bus: 消息总线实例
        agent_loop: AgentLoopAdapter 适配器实例
    ...
    def __init__(self):
        ...
        self.agent_loop: Optional[AgentLoopAdapter] = None
    ...
    async def initialize(self) -> bool:
        ...
        2. 创建消息总线
        3. 创建 LLM Provider
        4. 创建 AgentLoopAdapter 适配器（支持 LangGraph）
        ...
        try:
            ...
            # 根据 provider 类型选择 Provider 类
            if provider_name == "dashscope":
                # DashScope 使用自定义 Provider，支持视觉模型
                self.provider = DashScopeVisionProvider(
                ...
                logger.info("[Nanobot 服务] 使用 DashScopeVisionProvider（支持视觉模型）")
            else:
                # 其他 provider 使用 LiteLLMProvider
            ...
            # 创建 AgentLoop 适配器（支持 LangGraph 多模态）
            self.agent_loop = AgentLoopAdapter(
                bus=self.bus,
