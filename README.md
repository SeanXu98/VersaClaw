...
│      │   │   ├── channels/                          ├── models/    (数据模型)      │
│      │   │   ├── cron/                              ├── routers/   (API路由)       │
│      │   │   ├── system/                            ├── services/  (业务逻辑)      │
│      │   │   └── upload/                            ├── extension/ (扩展层)        │
│      │   ├── chat/                                 │   ├── scheduler.py    (模型调度)│
│      │   ├── models/                               │   ├── enhanced_loop.py (增强循环)│
│      │   ├── skills/                               │   ├── vision_agent.py  (视觉代理)│
│      │   ├── memory/                               │   ├── vision_agent_manager.py    │
│      │   ├── channels/                              │   ├── feature_analyzer.py       │
│      │   ├── cron/                                  │   └── config_extension.py        │
│      │   └── settings/                              └── utils/     (工具函数)      │
│      ├── components/                                       │                       │
│      │   ├── RightPanel.tsx                                ▼                       │
│      │   └── ImageUploader.tsx            ┌────────────────────────────────┐     │
...
| **Model Scheduler** | `app/extension/scheduler.py` | 智能模型调度器，支持模型选择和降级 |
| **Enhanced Agent Loop** | `app/extension/enhanced_loop.py` | 增强的 Agent 循环，支持多模态处理 |
| **Vision Agent** | `app/extension/vision_agent.py` | 视觉理解子代理配置和类型 |
| **Vision Agent Manager** | `app/extension/vision_agent_manager.py` | Vision Agent 管理器，执行图片分析 |
| **Feature Analyzer** | `app/extension/feature_analyzer.py` | 请求特征分析器，检测多模态请求 |
| **Config Extension** | `app/extension/config_extension.py` | 扩展配置，支持独立的 imageModel 配置 |
...
│                          ▼                                                      │
│              ┌───────────────────────┐                                         │
│              │   Feature Analyzer    │  ← 请求特征分析（检测图片、任务类型）       │
│              └───────────┬───────────┘                                         │
│                          ▼                                                      │
│              ┌───────────────────────┐                                         │
│              │   Model Scheduler     │  ← 智能模型调度（选择文本/视觉模型）        │
│              └───────────┬───────────┘                                         │
│                          ▼                                                      │
│         ┌────────────────┼────────────────┐                                    │
│         │                │                │                                     │
...
│  │ (AgentLoop) │  │  (规划中)    │  │ (VisionAgentManager)│                     │
│  └──────┬──────┘  └──────┬──────┘  └──────────┬──────────┘                     │
│         │                │                    │                                 │
│         └────────────────┼────────────────────┘                                 │
│                          ▼                                                      │
│              ┌───────────────────────┐                                         │
│              │   Enhanced Agent Loop │  ← 统一响应处理                           │
│              └───────────┬───────────┘                                         │
│                          ▼                                                      │
│         ┌────────────────┼────────────────┐                                    │
│         │                │                │                                     │
...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Feature Analyzer** | `extension/feature_analyzer.py` | 分析请求特征，检测图片、判断任务类型 |
| **Model Scheduler** | `extension/scheduler.py` | 智能模型选择，支持主模型/视觉模型切换和降级 |
| **Enhanced Agent Loop** | `extension/enhanced_loop.py` | 继承 AgentLoop，注入多模态处理逻辑 |
| **Vision Agent** | `extension/vision_agent.py` | 视觉代理配置和类型定义 |
| **Vision Agent Manager** | `extension/vision_agent_manager.py` | 管理视觉代理实例，执行图片分析 |
| **Config Extension** | `extension/config_extension.py` | 扩展配置，支持独立的 imageModel 配置 |
