...
const DEFAULT_MODELS: Record<string, { text: string[], vision: string[] }> = {
  zhipu: {
...
function isVisionModel(modelName: string): boolean {
  return VISION_PATTERNS.some(p => modelName.toLowerCase().includes(p))
}

/**
 * GET /api/models/available
 * 获取所有已配置的可用模型列表（代理到后端）
 */
export async function GET() {
  try {
    ...
    // 处理后端返回的提供商数据
    // 后端返回格式: { success: true, data: { all: [...], configured: [...] } }
    if (providersData.success && providersData.data?.all) {
      for (const providerInfo of providersData.data.all as any[]) {
        // 检查是否已配置（status 为 active）
        if (providerInfo.status === 'active') {
          const providerName = providerInfo.name
          const defaults = DEFAULT_MODELS[providerName] || { text: [], vision: [] }

          // 如果提供商配置了特定模型，使用配置的模型
          if (providerInfo.configured_models && providerInfo.configured_models.length > 0) {
            // 用户自定义模型列表，需要自动检测 Vision 能力
            const textModels: string[] = []
            const visionModels: string[] = []

            for (const model of providerInfo.configured_models as string[]) {
